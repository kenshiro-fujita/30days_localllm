...
高度な機械学習や情報工学の専門家ではないが、業務システム開発経験はあり、PythonやCLI、API、Gitなどに触れる基礎体力はある。

学習スタイルとしては、平日は1時間、土日祝は3時間を確保する。
...
|  Day 3 | 7/16 | 木  | 生成パラメータ                         | 1. 同じ質問をtemperature低め/高めで試す。<br>2. top_pの意味を調べる。<br>3. 事実回答、アイデア出し、文章生成で違いを見る。<br>4. 結果を表にする。                                                                                                                                                                                                                                  | temperature、top_p、確率的生成、再現性                        | 平日1h。成果物：`logs/day03_generation_params.md`                                                                                   |
...
|  Day 5 | 7/18 | 土  | モデル比較                           | 1. TinySwallowに `eval/questions.md` の10問を投げる。<br>2. 質問に素材が必要な場合は `eval/materials.md` の固定素材を使う。<br>3. 可能ならQwen2.5-1.5B-Instructや別の日本語対応モデルにも同一セットを投げる。<br>4. 自然さ、正確さ、指示追従性を評価する。<br>5. TinySwallowの得意・不得意をまとめる。                                                                                                                 | モデル比較、評価観点作成、日本語LLM観察                              | 休日3h。成果物：`eval/day05_model_compare.md`                                                                                       |
...
| Day 20 | 8/2  | 日  | RAG評価＋FT環境スモークテスト               | 【前半1.5h: RAG評価】<br>1. `eval/questions.md` からRAG向きの質問を選び、RAGなし/ありの回答を比較する。<br>2. 必要な素材は `eval/materials.md` を使う。<br>3. 根拠に沿っているか確認し、間違い方を分類する。<br><br>【後半1.5h: FT環境疎通】<br>4. Colab + Unslothの公式LoRAノートブックを、サンプルデータのまま一度最後まで実行する。<br>5. Macの場合はmlx-lmのLoRAチュートリアルも試せる範囲で試す。<br>6. Day26で使う本命ルートを決める。<br>7. 詰まった場合はColab一本化を即決する。 | RAG評価、根拠性、ハルシネーション観察、FT環境の事前検証                     | 休日3h。成果物：`eval/day20_rag_eval.md`, `finetune/env_smoke_result.md`。この日がFT失敗リスク対策の最重要日。                                        |
...
| Day 24 | 8/6  | 木  | シンセティックデータ生成①                   | 1. ChatGPT/Claude/Gemini等で、`messages` 形式JSONLの候補を30〜50件生成する。<br>2. 生成プロンプトを調整する。<br>3. 明らかに低品質な例を除外する。<br>4. 5件を評価用に分離する。                                                                                                                                                                                                      | シンセティックデータ生成、messages形式JSONL、品質確認、評価用データ分離         | 平日1h。成果物：`dataset/raw_generated_v0.jsonl`, `dataset/eval_seed.jsonl`                                                         |
| Day 25 | 8/7  | 金  | シンセティックデータ生成②                   | 1. `messages` 形式のデータを100件前後まで増やす。<br>2. 重複、表記ゆれ、長すぎる出力を修正する。<br>3. train/valid/evalに分ける。<br>4. JSONLとして読み込めるか検証スクリプトを作る。<br>5. FT評価用質問10問を `eval/ft_questions.md` として固定する。                                                                                                                                                     | データクリーニング、train/valid/eval分割、JSONL検証、FT評価設計        | 平日1h。成果物：`dataset/train.jsonl`, `dataset/valid.jsonl`, `dataset/eval.jsonl`, `src/validate_jsonl.py`, `eval/ft_questions.md` |
...
| Day 27 | 8/9  | 日  | 本番FT＋評価                         | 1. Day26で通った設定を使う。<br>2. 学習step、epoch、learning rateを決めて本番学習する。<br>3. base Qwenとtuned Qwenに `eval/ft_questions.md` の同一質問を投げる。<br>4. 良くなった点、悪くなった点、壊れた点を記録する。                                                                                                                                                                    | 本番LoRA/QLoRA、評価比較、ハイパーパラメータ観察                      | 休日3h。成果物：`finetune/tuned_adapter_v1/`, `eval/day27_ft_eval.md`                                                               |
| Day 28 | 8/10 | 月  | TinySwallowとの比較                 | 1. TinySwallow、base Qwen、tuned Qwenに `eval/questions.md` の同一質問を投げる。<br>2. 必要な素材は `eval/materials.md` を使う。<br>3. TinySwallowの自然さと、tuned Qwenのタスク特化を比較する。<br>4. FTで変わるもの・変わらないものを整理する。<br>5. 余力があれば、Day26〜27のスクリプトのモデル名を `SakanaAI/TinySwallow-1.5B-Instruct` に差し替えて数stepだけFTスモークを試す。                                            | モデル横断比較、FT効果の理解、同系統モデルへのFT転用観察                     | 平日1h。成果物：`eval/day28_tinyswallow_vs_tuned_qwen.md`。TinySwallow FTは余力課題であり、失敗しても本編の失敗扱いにしない。                                  |
| Day 29 | 8/11 | 火  | 最終評価と配備                         | 1. 必須タスクとして、`eval/questions.md` と `eval/ft_questions.md` を使い、TinySwallow、base Qwen、tuned Qwenを比較する。<br>2. 必要な素材は `eval/materials.md` を使う。<br>3. 最終評価を `eval/final_eval.md` にまとめる。<br>4. 任意タスクとして、残り時間でadapter mergeまたはGGUF変換を試す。<br>5. 任意タスクとして、Ollamaまたはllama.cppで使う流れを調べる。<br>6. READMEにローカルLLMとして使える状態をまとめる。                | 最終評価、モデル配備、GGUF/Ollama理解                           | 祝日3h。成果物：`eval/final_eval.md`。GGUF変換は任意。最終評価が完了していれば完走扱い。                                                                    |
...
Day 20で重要なのは、FT環境の不確実性をDay26まで持ち越さないことである。

---

# 16. 評価質問セット

## 16.1 `eval/questions.md`
...
10. MATERIAL-001〜004に書かれていないことを聞かれた場合は、分からないと答えてください。
```

## 16.2 `eval/materials.md`
...
## 16.3 `eval/ft_questions.md`

この質問セットは、FTで狙ったタスク特化性能を見るために使う。
Day 25で固定し、Day 27 / Day 29で使う。
